...
const navigationItems = [
  ...
  { id: "add", label: "Add transaction", shortLabel: "Add" },
  { id: "transactions", label: "Transactions", shortLabel: "Records" },
  { id: "budgets", label: "Budgets", shortLabel: "Budget" },
  { id: "analytics", label: "Analytics", shortLabel: "Reports" },
];
    ...
    <main className="min-h-screen overflow-x-hidden text-slate-950">
      <div className="flex min-h-screen">
        <div className="w-full min-w-0 flex-1">
          ...
          <section className="flex w-full flex-col gap-6 px-3 pb-24 pt-4 sm:gap-7 sm:px-6 sm:py-7 lg:px-8 xl:px-8 xl:pb-8 2xl:px-10">
          ...
          <nav
            className="fixed inset-x-2 bottom-2 z-40 grid grid-cols-5 gap-1 rounded-[1.25rem] border border-slate-200 bg-white/95 p-1.5 shadow-xl shadow-slate-950/15 backdrop-blur xl:hidden"
            aria-label="Quick section navigation"
          >
            {navigationItems.map((item) => (
              <button
                className={`min-h-12 min-w-0 rounded-2xl px-1 py-2 text-center text-[0.68rem] font-semibold leading-tight transition focus:outline-none focus:ring-4 focus:ring-slate-200 sm:text-xs ${
                  activeSection === item.id
                    ? "bg-slate-950 text-white"
                    : "text-slate-600 hover:bg-slate-100"
                }`}
